...
class BRNN_Matrix(L.LightningModule):
        ...
        **kwargs,
        ...
        if self.num_linear_layers == 1:
            self.output_layer = nn.Linear(
                in_features=lstm_hidden_size * 2,  # *2 for bidirection
                out_features=num_classes,
            )
        elif self.num_linear_layers == 2:
            self.linear_layers = nn.ModuleList()
            self.linear_layers.append(
                nn.Linear(lstm_hidden_size * 2, self.linear_hidden_size)
            )
            ...
            for i in range(1, self.num_linear_layers - 1):
                self.linear_layers.append(
                    nn.ReLU(nn.Linear(self.linear_hidden_size, self.linear_hidden_size))
                )
    ...
    def configure_optimizers(self):
        if self.optimizer_name == "SGD":
            ...
            raise ValueError(
                "Invalid optimizer name. Supported options: 'SGD', 'Adam', 'AdamW'."
            )
        return optimizer
