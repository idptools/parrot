...
def parse_file(tsvfile, datatype):
	'''
...
# Extended PyTorch Dataset class for sequence data
...
# Collate samples into a batch, zero-padding end of sequences to the same size
def unequal_len_collate(batch):
    ...
    padded_seq_vectors = np.zeros([len(orig_seq_vectors), longest_seq, 20])	# TODO: change '20' to work for any type of encoding scheme
...
def split_data(data_file, datatype, split_file=None, percent_val=0.15, percent_test=0.15):
	data = parse_file(data_file, datatype)
